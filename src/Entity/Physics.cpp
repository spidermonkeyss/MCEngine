...
void Physics::PushEntityOutOfBlock(BlockCollision blockCollision)
{
	if (blockCollision.blockFaceCollidedWith == BlockCollision::top && blockCollision.entity->velocity.y < 0)
	{
		...
		blockCollision.entity->transform.position.y = blockCollision.block->position.y + (1 - CollisionDetection::minimum_collision_distance);
	}
	...
	else if (blockCollision.blockFaceCollidedWith == BlockCollision::North && blockCollision.entity->velocity.z > 0)
	{
		...
		blockCollision.entity->transform.position.z = blockCollision.block->position.z - (1 - CollisionDetection::minimum_collision_distance);
	}
	...
	else if (blockCollision.blockFaceCollidedWith == BlockCollision::East && blockCollision.entity->velocity.x < 0)
	{
		...
		blockCollision.entity->transform.position.x = blockCollision.block->position.x + (1 - CollisionDetection::minimum_collision_distance);
	}
